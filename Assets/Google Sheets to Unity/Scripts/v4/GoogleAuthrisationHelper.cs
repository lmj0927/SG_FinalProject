...
namespace GoogleSheetsToUnity
{
    public class GoogleAuthrisationHelper : MonoBehaviour
    {
        ...
        static void GetAuthComplete(string authToken)
        {
            ...
            f.AddField("client_id", SpreadsheetManager.Config.CLIENT_ID.Trim());
            f.AddField("client_secret", SpreadsheetManager.Config.CLIENT_SECRET.Trim());
        ...
        static IEnumerator GetToken(WWWForm f)
        {
            using (UnityWebRequest request = UnityWebRequest.Post("https://accounts.google.com/o/oauth2/token", f))
            {
                ...
                {
                    Debug.LogError("[GSTU] OAuth token exchange failed (HTTP).\n" + request.error + "\n" + responseText);
                    yield break;
                }

                GoogleDataResponse tokenResponse = JsonUtility.FromJson<GoogleDataResponse>(responseText);
                if (tokenResponse == null || string.IsNullOrEmpty(tokenResponse.access_token))
                {
                    Debug.LogError("[GSTU] OAuth token exchange failed (empty access_token).\n" + responseText);
                    yield break;
                }

                SpreadsheetManager.Config.gdr = tokenResponse;
                SpreadsheetManager.Config.gdr.nextRefreshTime =
                    DateTime.Now.AddSeconds(Mathf.Max(tokenResponse.expires_in, 60));
                EditorUtility.SetDirty(SpreadsheetManager.Config);
                AssetDatabase.SaveAssets();
                Debug.Log("[GSTU] OAuth connected. access_token saved. refresh_token present: "
                    + !string.IsNullOrEmpty(SpreadsheetManager.Config.gdr.refresh_token));
            }
        }
        ...
        {
            EnsureLegacyRefreshTokenMigrated();
            ...
            using (UnityWebRequest request = UnityWebRequest.Post("https://accounts.google.com/oauth2/token", f))
            {
                yield return request.SendWebRequest();

                string responseText = request.downloadHandler.text;
                GoogleDataResponse newGdr = JsonUtility.FromJson<GoogleDataResponse>(responseText);

                if (request.result != UnityWebRequest.Result.Success
                    || newGdr == null
                    || string.IsNullOrEmpty(newGdr.access_token))
                {
                    Debug.LogError("[GSTU] Token refresh failed.\n" + request.error + "\n" + responseText);
                    yield break;
                }

                SpreadsheetManager.Config.gdr.access_token = newGdr.access_token;
                SpreadsheetManager.Config.gdr.nextRefreshTime =
                    DateTime.Now.AddSeconds(Mathf.Max(newGdr.expires_in, 60));
        ...
        static void EnsureLegacyRefreshTokenMigrated()
        {
            if (!string.IsNullOrEmpty(SpreadsheetManager.Config.gdr.refresh_token))
            {
                return;
            ...
            if (string.IsNullOrEmpty(SpreadsheetManager.Config.REFRESH_TOKEN))
            {
                return;
            }

            SpreadsheetManager.Config.gdr.refresh_token = SpreadsheetManager.Config.REFRESH_TOKEN.Trim();
#if UNITY_EDITOR
            EditorUtility.SetDirty(SpreadsheetManager.Config);
            AssetDatabase.SaveAssets();
#endif
